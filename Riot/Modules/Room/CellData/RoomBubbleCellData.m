...
-(MXRoomPowerLevels*)getPreviousPowerLevels:(MXEvent *)specificEvent withRoomDataSource:(MXKRoomDataSource *)dataSource
{
    NSArray *bubbles = [dataSource valueForKey:@"bubbles"];
    NSArray *toProcess = [dataSource valueForKey:@"eventsToProcess"];
    NSArray *toProcessSnapshot = [dataSource valueForKey:@"eventsToProcessSnapshot"];
    MXEvent *priorPowerEvent = nil;
    bool found = false;
    
    //find the most recent power event
    for (long i = bubbles.count - 1; i > 0; i--){
        MXKRoomBubbleCellData *bubble = (MXKRoomBubbleCellData*)bubbles[i];
        for (long j = bubble.events.count - 1; j > 0; j--){
            if (bubble.events[j].originServerTs < specificEvent.originServerTs && bubble.events[j].eventType == MXEventTypeRoomPowerLevels && [self powerLevelsDifferent:[self getPowerLevelsForEvent:bubble.events[j]] and:[self getPowerLevelsForEvent:specificEvent]] && (!priorPowerEvent || bubble.events[j].originServerTs > priorPowerEvent.originServerTs)){
                priorPowerEvent = bubble.events[j];
            }
        }
    }
    found = false;
    for (long i = toProcess.count - 1; i > 0; i--){
        MXKQueuedEvent *queued = toProcess[i];
        if (queued.event.originServerTs < specificEvent.originServerTs && queued.event.eventType == MXEventTypeRoomPowerLevels && [self powerLevelsDifferent:[self getPowerLevelsForEvent:queued.event] and:[self getPowerLevelsForEvent:specificEvent]] && (!priorPowerEvent || queued.event.originServerTs > priorPowerEvent.originServerTs)){
            if (priorPowerEvent){
                //we want the most recent event that is not the event we are checking against
                if (queued.event.originServerTs > priorPowerEvent.originServerTs){
                    priorPowerEvent = queued.event;
                    continue;
                } else {
                    continue;
                }
    ...
    found = false;
    for (long i = toProcessSnapshot.count - 1; i > 0; i--){
        MXKQueuedEvent *queued = toProcessSnapshot[i];
        if (queued.event.originServerTs < specificEvent.originServerTs && queued.event.eventType == MXEventTypeRoomPowerLevels && [self powerLevelsDifferent:[self getPowerLevelsForEvent:queued.event] and:[self getPowerLevelsForEvent:specificEvent]] && (!priorPowerEvent || queued.event.originServerTs > priorPowerEvent.originServerTs)){
            if (priorPowerEvent){
                //we want the most recent event that is not the event we are checking against
                if (queued.event.originServerTs > priorPowerEvent.originServerTs){
    ...
    if (priorPowerEvent){
        MXRoomPowerLevels *powerLevels = [MXRoomPowerLevels modelFromJSON:priorPowerEvent.content];
        return powerLevels;
    }
...
-(void)injectEventData:(MXEvent *)specificEvent withRoomDataSource:(MXKRoomDataSource *)rDataSource
{
    if (specificEvent.eventType == MXEventTypeRoomPowerLevels){
        MXRoomPowerLevels *old = [self getPreviousPowerLevels:specificEvent withRoomDataSource:rDataSource];
        //MXRoomPowerLevels *powerLevels = [MXRoomPowerLevels modelFromJSON:specificEvent.content];
        /*if (![powerLevels.users isEqual: roomState.powerLevels.users]){
            NSLog(@"Viable Strategy");
        }*/
        //inject the previous power levels into here
        if (old)
        {
            NSMutableDictionary *dictionary = [NSMutableDictionary new];
            [dictionary setObject:old.users forKey:@"previousPowerLevels"];
            [specificEvent setValue:dictionary forKey:@"_prevContent"];
            NSLog(@"Injected data");
        }
    }
}

-(MXRoomPowerLevels*)getPowerLevelsForEvent:(MXEvent*)event
{
    if (event.eventType == MXEventTypeRoomPowerLevels){
        return [MXRoomPowerLevels modelFromJSON:event.content];
    }
    return nil;
}

-(bool)powerLevelsDifferent:(MXRoomPowerLevels*)left and:(MXRoomPowerLevels*)right
{
    for (NSString* key in [left.users allKeys]){
        if (left.users[key] && right.users[key]){
            if ([left.users[key] integerValue] != [right.users[key] integerValue]){
                return true;
            }
        }
    }
...
- (instancetype)initWithEvent:(MXEvent *)event andRoomState:(MXRoomState *)roomState andRoomDataSource:(MXKRoomDataSource *)roomDataSource2
{
    ...
    [self injectEventData:event withRoomDataSource:roomDataSource2];
